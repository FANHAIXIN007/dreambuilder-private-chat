...
export default function PrivateChatWalletLoginPanel({
  ...
  const [walletMenuStyle, setWalletMenuStyle] = useState<CSSProperties>({});
  ...
  function updateWalletMenuPosition() {
    if (typeof window === "undefined") return;
  ...
  async function handleDisconnectWallet() {
    ...
    try {
      ...
      const nextLoginProvider =
        currentSession.email
          ...
          : currentSession.wechatOpenid
            ? "wechat"
            : (unbindResult.data
                ?.loginProvider as AppAccountSession["loginProvider"]) ||
      ...
      const message = error instanceof Error ? error.message : "解绑钱包失败。";
  ...
  return (
    <div className="relative z-[80] overflow-visible rounded-3xl border border-white/10 bg-white/[0.04] p-4">
      <div className="mb-3">
        <h3 className="text-sm font-semibold text-white">钱包登录 / 绑定</h3>
        <p className="mt-1 text-xs leading-5 text-slate-400">
          如果 DreamBuilder
          已经连接钱包，这里会显示已绑定钱包；如果聊天平台独立运行，也可以在这里单独连接钱包并签名登录。
        </p>
      </div>
      ...
      {hasWalletBound ? (
        ...
        <div className="space-y-3">
          ...
          {!address ? (
            <div className="relative z-[100] overflow-visible" ref={menuRef}>
              <button
                type="button"
              ...
              {open ? (
                <div
                  style={walletMenuStyle}
                  className="max-h-[min(420px,70vh)] overflow-y-auto rounded-3xl border border-white/10 bg-slate-950/95 p-2 shadow-[0_24px_80px_rgba(0,0,0,0.55)] ring-1 ring-white/[0.08] backdrop-blur-xl"
                >
                  ...
                  {walletOptions.map((wallet) => (
                    <button
                      ...
                      disabled={isConnecting}
                      className="flex w-full items-start justify-between rounded-2xl px-3 py-3 text-left transition hover:bg-white/[0.06] disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      <span>
                  ...
                  {!injectedConnector ? (
                    <div className="px-3 py-2 text-[11px] leading-5 text-rose-300">
                      手机上请使用 OKX Wallet、币安钱包、MetaMask 等钱包 App
                      自带浏览器打开本页面；普通手机浏览器暂时不能直接连接钱包。
                    </div>
                  ) : null}
                </div>
              ) : null}
          ...
          {connectError ? (
            <div className="rounded-2xl bg-rose-500/10 px-3 py-2 text-xs leading-5 text-rose-100">
              当前浏览器没有检测到可用钱包。手机上请使用 OKX
              Wallet、币安钱包、MetaMask 等钱包 App
              自带浏览器打开本页面；电脑上请安装并启用钱包插件。
            </div>
          ) : null}
        </div>
      )}
